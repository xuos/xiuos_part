...
struct mg_fd* mg_fs_open(struct mg_fs* fs, const char* path, int flags)
{
    ...
    if (fd != NULL) {
        fd->fd = calloc(1, sizeof(int));
        *(int*)fd->fd = PrivOpen(path, 0);
        MG_DEBUG(("open path: %s, fd: %d", path, *(int*)fd->fd));
        fd->fs = fs;
        if (fd->fd == NULL) {
            free(fd);
            fd = NULL;
...
{
    MG_DEBUG(("%s fatstat for %s", __func__, path));
    FILINFO fi;
    if (path[0] == '\0') {
        if (size)
            *size = 0;
        ...
        MG_DEBUG(("%s successful fatstat, file size: %u", __func__, *size));
...
static size_t p_read(void* fp, void* buf, size_t len)
{
    ...
    int read_len = PrivRead(*(int*)fp, buf, len);
    MG_DEBUG(("read len: %d", read_len));
    return read_len;
}

static size_t p_write(void* fp, const void* buf, size_t len)
{
    int write_len = PrivWrite(*(int*)fp, buf, len);
    MG_DEBUG(("write len: %d", write_len));
    return write_len;
}

static size_t p_seek(void* fp, size_t offset)
{
    MG_DEBUG(("lseek offset: %d", offset));
    if (PrivLseek(*(int*)fp, (long)offset, SEEK_SET) != 0)
        (void)0;
    return PrivLseek(*(int*)fp, 0, SEEK_CUR);
}
    ...
    struct mg_http_part* part)
{
    ...
    for (;;) {
        ...
        if (part != NULL && h1 + cd.len + 2 < h2 && s[h1 + cd.len] == ':' && mg_ncasecmp(&s[h1], cd.ptr, cd.len) == 0) {
...
int mg_http_parse(const char* s, size_t len, struct mg_http_message* hm)
{
    ...
    if (hm->body.len == (size_t)~0 && !is_response && mg_vcasecmp(&hm->method, "PUT") != 0 && mg_vcasecmp(&hm->method, "POST") != 0) {
        hm->body.len = 0;
        hm->message.len = (size_t)req_len;
    }

    // The 204 (No content) responses also have 0 body length
    if (hm->body.len == (size_t)~0 && is_response && mg_vcasecmp(&hm->uri, "204") == 0) {
        hm->body.len = 0;
        hm->message.len = (size_t)req_len;
    }
    ...
    const struct mg_http_serve_opts* opts)
{
    ...
    } else if (mg_http_etag(etag, sizeof(etag), size, mtime) != NULL && (inm = mg_http_get_header(hm, "If-None-Match")) != NULL && mg_vcasecmp(inm, etag) == 0) {
        mg_fs_close(fd);
...
{
    const char* sort_js_code = "<script>function srt(tb, sc, so, d) {"
    ...
    const char* sort_js_code2 = "for (var i = 0; i < tr.length; i++) tb.appendChild(tr[i]); "
    ...
    char* path, size_t path_size)
{
    ...
    if (flags == 0) {
    ...
    } else if ((flags & MG_FS_DIR) && hm->uri.len > 0 && hm->uri.ptr[hm->uri.len - 1] != '/') {
        mg_printf(c,
            "HTTP/1.1 301 Moved\r\n"
            "Location: %.*s/\r\n"
            "Content-Length: 0\r\n"
        ...
        if (((mg_snprintf(path + n, path_size - n, "/" MG_HTTP_INDEX) > 0 && (tmp = fs->st(path, NULL, NULL)) != 0) || (mg_snprintf(path + n, path_size - n, "/index.shtml") > 0 && (tmp = fs->st(path, NULL, NULL)) != 0))) {
            flags = tmp;
        } else if ((mg_snprintf(path + n, path_size - n, "/" MG_HTTP_INDEX ".gz") > 0 && (tmp = fs->st(path, NULL, NULL)) != 0)) { // check for gzipped index
            flags = tmp;
...
#endif
    } else if (flags && sp != NULL && mg_globmatch(sp, strlen(sp), path, strlen(path))) {
...
{
    size_t i = 0, n;
    while (i < s.len && is_hex_digit(s.ptr[i]))
        i++;
    n = mg_unhexn(s.ptr, i);
...
bool mg_json_get_num(struct mg_str json, const char* path, double* v)
{
    ...
    if ((n = mg_json_get(json, path, &toklen)) >= 0 && (json.ptr[n] == '-' || (json.ptr[n] >= '0' && json.ptr[n] <= '9'))) {
        if (v != NULL)
...
char* mg_json_get_str(struct mg_str json, const char* path)
{
    ...
    if (off >= 0 && len > 1 && json.ptr[off] == '"') {
        if ((result = (char*)calloc(1, (size_t)len)) != NULL && !mg_json_unescape(mg_str_n(json.ptr + off + 1, (size_t)(len - 2)), result, (size_t)len)) {
            free(result);
            result = NULL;
        }
    }
...
char* mg_json_get_b64(struct mg_str json, const char* path, int* slen)
{
    char* result = NULL;
    int len = 0, off = mg_json_get(json, path, &len);
    if (off >= 0 && json.ptr[off] == '"' && len > 1 && (result = (char*)calloc(1, (size_t)len)) != NULL) {
...
char* mg_json_get_hex(struct mg_str json, const char* path, int* slen)
{
    char* result = NULL;
    int len = 0, off = mg_json_get(json, path, &len);
    if (off >= 0 && json.ptr[off] == '"' && len > 1 && (result = (char*)calloc(1, (size_t)len / 2)) != NULL) {
...
static bool mg_aton6(struct mg_str str, struct mg_addr* addr)
{
    ...
    if (mg_v4mapped(str, addr))
        return true;
    for (i = 0; i < str.len; i++) {
        if ((str.ptr[i] >= '0' && str.ptr[i] <= '9') || (str.ptr[i] >= 'a' && str.ptr[i] <= 'f') || (str.ptr[i] >= 'A' && str.ptr[i] <= 'F')) {
            unsigned long val;
            if (i > j + 3)
                return false;
...
{
    pkt->pay = mg_str_n((char*)p, (size_t)(&pkt->raw.ptr[pkt->raw.len] - (char*)p));
}
...
{
    struct ip* ip = tx_ip(ifp, mac_dst, 17, ip_src, ip_dst, len + sizeof(struct udp));
...
static void rx_dhcp_client(struct mg_tcpip_if* ifp, struct pkt* pkt)
{
    ...
    } else if (msgtype == 2 && ifp->state == MG_TCPIP_STATE_UP && ip && gw && lease) { // DHCPOFFER
...
{
    struct ip* ip = tx_ip(ifp, dst_mac, 6, ifp->ip, dst_ip, sizeof(struct tcp) + len);
...
static size_t trim_len(struct mg_connection* c, size_t len)
{
    ...
    size_t max_headers_len = eth_h_len + ip_max_h_len + (c->is_udp ? udp_h_len : tcp_max_h_len);
...
static void read_conn(struct mg_connection* c, struct pkt* pkt)
{
    ...
    tx_tcp((struct mg_tcpip_if *) c->mgr->priv, s->mac, rem_ip, TH_ACK, c->loc.port,
        ...
        if (c->is_tls) {
            ...
            io = &c->recv;
            if (io->size - io->len < pkt->pay.len && !mg_iobuf_resize(io, io->len + pkt->pay.len)) {
                mg_error(c, "oom");
            } else {
...
static void mg_tcpip_poll(struct mg_tcpip_if* ifp, uint64_t uptime_ms)
{
    ...
    } else if (expired_1000ms && ifp->state == MG_TCPIP_STATE_READY && ifp->lease_expire > 0) { // BOUND / RENEWING / REBINDING
    ...
    if (ifp->driver->rx != NULL) { // Polling driver. We must call it
        size_t len = ifp->driver->rx(ifp->recv_queue.buf, ifp->recv_queue.size, ifp);
        if (len > 0)
...
void mg_tcpip_init(struct mg_mgr* mgr, struct mg_tcpip_if* ifp)
{
    ...
    if (ifp->driver->init && !ifp->driver->init(ifp)) {
        ...
        if (ifp->tx.ptr == NULL || ifp->recv_queue.buf == NULL)
            MG_ERROR(("OOM"));
    }
}
...
{
    struct connstate* s = (struct connstate*)(c + 1);
    if (c->is_udp == false && c->is_listening == false && c->is_connecting == false) { // For TCP conns,
        struct mg_tcpip_if* ifp = (struct mg_tcpip_if*)c->mgr->priv; // send TCP FIN
        uint32_t rem_ip;
        memcpy(&rem_ip, c->rem.ip, sizeof(uint32_t));
...
static struct mg_otadata mg_otadata(int fw)
{
    ...
    if ((fw == MG_FIRMWARE_CURRENT && bank == 2))
        key++;
    if ((fw == MG_FIRMWARE_PREVIOUS && bank == 1))
        key++;
...
static void read_conn(struct mg_connection* c)
{
    ...
    if (c->recv.len >= MG_MAX_RECV_SIZE) {
        mg_error(c, "max_recv_buf_size reached");
    } else if (c->recv.size <= c->recv.len && !mg_iobuf_resize(&c->recv, c->recv.size + MG_IO_SIZE)) {
        mg_error(c, "oom");
    } else {
...
{
    return (((uint64_t)mg_swap32((uint32_t)v)) << 32) | mg_swap32((uint32_t)(v >> 32));
}
...
void mg_tls_handshake(struct mg_connection* c)
{
    ...
    if (rio->len < (size_t)TLS_HDR_SIZE + record_len)
        return;
    // Got full hello
...
{
    int rc;
    if (str.ptr == NULL || str.ptr[0] == '\0' || str.ptr[0] == '*')
        return true;
    if (str.ptr[0] == '-')
...
void mg_tls_handshake(struct mg_connection* c)
{
    ...
    } else if (rc == MBEDTLS_ERR_SSL_WANT_READ || rc == MBEDTLS_ERR_SSL_WANT_WRITE) { // Still pending
...
static STACK_OF(X509_INFO) * load_ca_certs(struct mg_str ca)
{
    ...
    STACK_OF(X509_INFO)* certs = bio ? PEM_X509_INFO_read_bio(bio, NULL, NULL, NULL) : NULL;
    if (bio)
        BIO_free(bio);
    return certs;
}
...
uint64_t mg_millis(void)
{
    ...
    return (uint64_t)((CurrentTicksGain() * 1000) / TICK_PER_SECOND);
...
enum { IMXRT1020_PHY_ADDR = 0x02,
    IMXRT1020_PHY_BCR = 0,
    IMXRT1020_PHY_BSR = 1 }; // PHY constants
...
static bool mg_tcpip_driver_imxrt1020_init(struct mg_tcpip_if* ifp)
{
    ...
    while (imxrt1020_eth_read_phy(IMXRT1020_PHY_ADDR, IMXRT1020_PHY_BSR) & BIT(15)) {
        delay(0x5000);
    } // Wait finished poll 10ms
...
static size_t mg_tcpip_driver_imxrt1020_tx(const void* buf, size_t len, struct mg_tcpip_if* ifp)
{
    ...
    if (len > sizeof(tx_data_buffer[ENET_TXBD_NUM])) {
        ...
        if (++s_rt1020_txno >= ENET_TXBD_NUM)
            s_rt1020_txno = 0;
    }
    (void)ifp;
    return len;
    ...
    if (eir & EIMR_RX_ERR) // Global mask used
    {
        ...
        // Read inframes
            ...
            for (uint32_t i = 0; i < 10; i++) { // read as they arrive but not forever
                ...
                if (++s_rt1020_rxno >= ENET_RXBD_NUM)
                    s_rt1020_rxno = 0;
            }
        }
    }
...
{
    struct mg_tcpip_driver_same54_data* d = (struct mg_tcpip_driver_same54_data*)ifp->driver_data;
    s_ifp = ifp;
    ...
    GMAC_REGS->SA[0].GMAC_SAB = MG_U32(ifp->mac[3], ifp->mac[2], ifp->mac[1], ifp->mac[0]);
...
void GMAC_Handler(void)
{
    ...
    if ((tsr & (GMAC_TSR_HRESP_Msk | GMAC_TSR_UND_Msk | GMAC_TSR_TXCOMP_Msk | GMAC_TSR_TFC_Msk | GMAC_TSR_TXGO_Msk | GMAC_TSR_RLE_Msk | GMAC_TSR_COL_Msk | GMAC_TSR_UBR_Msk)) != 0) {
        ...
        if (!(s_txdesc[s_txno][1] & BIT(31)))
            s_txdesc[s_txno][1] |= BIT(31);
    }
...
{
    struct mg_tcpip_driver_stm32_data* d = (struct mg_tcpip_driver_stm32_data*)ifp->driver_data;
    s_ifp = ifp;
    ...
    for (int i = 0; i < ETH_DESC_CNT; i++) {
        ...
        s_rxdesc[i][3] = (uint32_t)(uintptr_t)s_rxdesc[(i + 1) % ETH_DESC_CNT]; // Chain
    }

    // Init TX descriptors
    for (int i = 0; i < ETH_DESC_CNT; i++) {
        s_txdesc[i][2] = (uint32_t)(uintptr_t)s_txbuf[i]; // Buf pointer
        s_txdesc[i][3] = (uint32_t)(uintptr_t)s_txdesc[(i + 1) % ETH_DESC_CNT]; // Chain
    }
...
void ETH_IRQHandler(void)
{
    if (ETH->DMASR & BIT(6)) { // Frame received, loop
        ...
        for (uint32_t i = 0; i < 10; i++) { // read as they arrive but not forever
            if (s_rxdesc[s_rxno][0] & BIT(31))
                break; // exit when done
            if (((s_rxdesc[s_rxno][0] & (BIT(8) | BIT(9))) == (BIT(8) | BIT(9))) && !(s_rxdesc[s_rxno][0] & BIT(15))) { // skip partial/errored frames
...
#if MG_ENABLE_TCPIP && defined(MG_ENABLE_DRIVER_STM32H) && MG_ENABLE_DRIVER_STM32H
struct stm32h_eth {
...
static uint32_t get_hclk(void)
{
    ...
    if (sel == 1) {
        ...
        n = ((rcc->PLL1DIVR & (0x1FF << 0)) >> 0) + 1 + ((rcc->PLLCFGR & BIT(0)) ? 1 : 0); // round-up in fractional mode
...
{
    struct mg_tcpip_driver_stm32h_data* d = (struct mg_tcpip_driver_stm32h_data*)ifp->driver_data;
    s_ifp = ifp;
    ...
    eth_write_phy(PHY_ADDR, PHY_BCR, BIT(15)); // Reset PHY
    eth_write_phy(PHY_ADDR, PHY_BCR, BIT(12)); // Set autonegotiation
    ETH->DMACRDLAR = (uint32_t)(uintptr_t)s_rxdesc; // RX descriptors start address
    ETH->DMACRDRLR = ETH_DESC_CNT - 1; // ring length
    ETH->DMACRDTPR = (uint32_t)(uintptr_t)&s_rxdesc[ETH_DESC_CNT - 1]; // last valid descriptor address
    ETH->DMACTDLAR = (uint32_t)(uintptr_t)s_txdesc; // TX descriptors start address
    ETH->DMACTDRLR = ETH_DESC_CNT - 1; // ring length
    ETH->DMACTDTPR = (uint32_t)(uintptr_t)s_txdesc; // first available descriptor address
    ETH->DMACCR = 0; // DSL = 0 (contiguous descriptor table) (reset value)
    ETH->DMACIER = BIT(6) | BIT(15); // RIE, NIE
    ETH->MACCR = BIT(0) | BIT(1) | BIT(13) | BIT(14) | BIT(15); // RE, TE, Duplex, Fast, Reserved
...
void ETH_IRQHandler(void)
{
    if (ETH->DMACSR & BIT(6)) { // Frame received, loop
        ...
        for (uint32_t i = 0; i < 10; i++) { // read as they arrive but not forever
            if (s_rxdesc[s_rxno][3] & BIT(31))
                break; // exit when done
            if (((s_rxdesc[s_rxno][3] & (BIT(28) | BIT(29))) == (BIT(28) | BIT(29))) && !(s_rxdesc[s_rxno][3] & BIT(15))) { // skip partial/errored frames
    ...
    ETH->DMACRDTPR = (uint32_t)(uintptr_t)&s_rxdesc[ETH_DESC_CNT - 1]; // and resume RX
}
...
{
    struct mg_tcpip_driver_tm4c_data* d = (struct mg_tcpip_driver_tm4c_data*)ifp->driver_data;
    s_ifp = ifp;
    ...
    while ((EMAC->EMACDMABUSMOD & BIT(0)) != 0)
    ...
    EMAC->EMACDMAOPMODE = BIT(1) | BIT(13) | BIT(21) | BIT(25); // SR, ST, TSF, RSF
    ...
    struct mg_tcpip_if* ifp)
{
    if (len > sizeof(s_txbuf[s_txno])) {
        ...
        memcpy(s_txbuf[s_txno], buf, len); // Copy data
        s_txdesc[s_txno][1] = (uint32_t)len; // Set data len
        s_txdesc[s_txno][0] = BIT(20) | BIT(28) | BIT(29) | BIT(30); // Chain,FS,LS,IC
        s_txdesc[s_txno][0] |= BIT(31); // Set OWN bit - let DMA take over
        if (++s_txno >= ETH_DESC_CNT)
            s_txno = 0;
    }
