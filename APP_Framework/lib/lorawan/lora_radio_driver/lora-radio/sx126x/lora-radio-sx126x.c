...
bool RadioInit( RadioEvents_t *events )
{
    ...
    if (lora_radio_init == false) {
        ...
        PrivTaskCreate(&lora_radio_task, &attr, &lora_radio_thread_entry, &args);
...
void RadioOnTxTimeoutIrq( void /** context*/ )
{
    ...
    LORA_RADIO_DEBUG_LOG(LR_DBG_INTERFACE, LOG_LEVEL, "%s PHY TX Timeout\r", __func__);
}
...
void RadioOnRxTimeoutIrq( void /** context*/ )
{
    ...
    LORA_RADIO_DEBUG_LOG(LR_DBG_INTERFACE, LOG_LEVEL, "%s PHY RX Timeout\r", __func__);
}
...
#ifndef LORA_RADIO_DRIVER_USING_RTOS_XIUOS
void RadioIrqProcess( void )
{
#ifdef LORA_RADIO_DRIVER_USING_ON_RTOS_RT_THREAD
