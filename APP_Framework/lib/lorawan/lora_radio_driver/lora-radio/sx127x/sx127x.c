...
void RadioIrqProcess( uint8_t irq_index )
{    
    uint16_t reg_irq_flags = SX127xRead(REG_LR_IRQFLAGS);
    
    if (reg_irq_flags & RFLR_IRQFLAGS_RXDONE) {
        irq_index = 0;//DIO0 RXDONE
    } else if (reg_irq_flags & RFLR_IRQFLAGS_RXTIMEOUT) {
        irq_index = 1;//DIO0 RXTIMEOUT
    } else if (reg_irq_flags & RFLR_IRQFLAGS_TXDONE) {
        irq_index = 0;//DIO0 TXDONE
    }
    #if 0 
    else if (reg_irq_flags & RFLR_IRQFLAGS_FHSSCHANGEDCHANNEL) {
        irq_index = 2;//DIO2 FHSSCHANGEDCHANNEL
    } else if ((reg_irq_flags & RFLR_IRQFLAGS_CADDONE) || (reg_irq_flags & RFLR_IRQFLAGS_CADDETECTED)) {
        irq_index = 3;//DIO3 CADDONE or CADDETECTED
    } 
    #endif 
    else {
        return;
