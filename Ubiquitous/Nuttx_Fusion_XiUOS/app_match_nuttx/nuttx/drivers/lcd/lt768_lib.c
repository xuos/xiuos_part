...
uint8_t lt768_cclk;    // LT768的内核时钟频率
uint8_t lt768_mclk;    // SDRAM的时钟频率
uint8_t lt768_sclk;    // LCD的扫描时钟频率
...
// check lt768 reset status
void lt768_sys_check(void)
{
    uint8_t val = 0;
    ...
    while(1)
    {
        ...
        {
            up_mdelay(1);                  //若MCU 速度太快，必要時使用
            LCD_CmdWrite(0x01);
            up_mdelay(1);                  //若MCU 速度太快，必要時使用
            val = LCD_DataRead();
            syslog(LOG_NOTICE, "LCD mdelay 1 [%x]\r\n", val);

            if((val & 0x80) == 0x80)       //检测CCR寄存器PLL是否准备好
            {
                break;
            }
            else
...
(
    uint16_t XCenter           // 圆心X位置
    ,uint16_t YCenter           // 圆心Y位置
    ,uint16_t R                 // 半径
...
(
    uint16_t XCenter          // 椭圆心X位置
    ,uint16_t YCenter          // 椭圆心Y位置
    ,uint16_t X_R              // 宽半径
    ,uint16_t Y_R              // 长半径
...
(
    uint16_t XCenter           // 曲心X位置
    ,uint16_t YCenter           // 曲心Y位置
    ,uint16_t X_R               // 宽半径
    ,uint16_t Y_R               // 长半径
...
(
    ...
    uint16_t width1,              // 内框宽度
    uint16_t width2,              // 外框宽度
    uint8_t  mode                 // 0：项目栏纵向   1：项目栏横向
)
{
    uint16_t i = 0;
    uint16_t x2,y2;
...
(
    uint8_t Size         // 设置字体大小  16：16*16     24:24*24    32:32*32
    ,uint8_t XxN          // 字体的宽度放大倍数：1~4
    ,uint8_t YxN          // 字体的高度放大倍数：1~4
    ,uint8_t ChromaKey    // 0：字体背景色透明    1：可以设置字体的背景色
    ,uint8_t Alignment    // 0：不字体不对齐      1：字体对齐
)
{
    if(Size==16)
    {
...
(
    uint16_t x               // 字体开始显示的x位置
    ,uint16_t y               // 字体开始显示的y位置
...
(
    ...
    ,uint16_t YS0                // S0图像的左上方Y坐标
    ,unsigned long Des_Addr            // 目的图像的内存起始地址
    ,uint16_t Des_W              // 目的图像的宽度
    ,uint16_t XDes               // 目的图像的左上方X坐标(显示窗口的起始x坐标)
    ,uint16_t YDes               // 目的图像的左上方Y坐标(显示窗口的起始y坐标)
    ,uint16_t X_W                // 显示窗口的宽度
    ,uint16_t Y_H                // 显示窗口的长度
...
(
    ...
    ,uint16_t width               // 显示层的宽度
    ,uint8_t Size              // 设置字体大小  48：48*48     72:72*72
    ,uint8_t ChromaKey         // 0：字体背景色透明    1：可以设置字体的背景色
    ,uint16_t x                   // 字体开始显示的x位置
    ,uint16_t y                   // 字体开始显示的y位置
    ,unsigned long FontColor            // 字体的颜色
    ,unsigned long BackGroundColor      // 字体的背景色（注意：当字体背景初始化成透明时，设置该值无效）
    ,uint16_t w          // 字体粗细：0：不加粗  1：加粗1级  2：加粗2级
    ,uint16_t s                   // 行距
    ,uint8_t *c                   // 数据缓冲的首地址
)
{
...
(
    uint8_t On_Off_Blinking         // 0：禁止光标闪烁   1：使能光标闪烁
    ,uint16_t Blinking_Time          // 设置文字光标闪烁时间
    ,uint16_t X_W                    // 文字光标水平大小
    ,uint16_t Y_W                    // 文字光标垂直大小
)
{
...
(
    uint8_t Cursor_N                  // 选择光标   1:光标1   2:光标2   3:光标3  4:光标4
    ,uint8_t Color1                    // 颜色1
    ,uint8_t Color2                    // 颜色2
    ,uint16_t X_Pos                    // 显示坐标X
    ,uint16_t Y_Pos                    // 显示坐标Y
    ,uint8_t *Cursor_Buf               // 光标数据的缓冲首地址
)
{
    uint32_t i ;
...
void LT768_PIP_Init
(
    uint8_t On_Off         // 0 : 禁止 PIP    1 : 使能 PIP    2 : 保持原来的状态
    ,uint8_t Select_PIP     // 1 : 使用 PIP1   2 : 使用 PIP2
    ,unsigned long PAddr          // PIP的开始地址
    ,uint16_t XP            // PIP窗口的X坐标,必须被4整除
    ,uint16_t YP            // PIP窗口的Y坐标,必须被4整除
    ,unsigned long ImageWidth     // 底图的宽度
    ,uint16_t X_Dis         // 显示窗口的X坐标
    ,uint16_t Y_Dis         // 显示窗口的Y坐标
    ,uint16_t X_W           // 显示窗口的宽度，必须被4整除
    ,uint16_t Y_H           // 显示窗口的长度，必须被4整除
)
{
    if(Select_PIP == 1)
    {
...
(
    unsigned long S0_Addr     // SO图像的内存起始地址
    ,uint16_t S0_W       // S0图像的宽度
    ,uint16_t XS0        // S0图像的左上方X坐标
    ,uint16_t YS0        // S0图像的左上方Y坐标
    ,unsigned long S1_Addr     // S1图像的内存起始地址
    ,uint16_t S1_W       // S1图像的宽度
    ,uint16_t XS1        // S1图像的左上方X坐标
    ,uint16_t YS1        // S1图像的左上方Y坐标
    ,unsigned long Des_Addr    // 目的图像的内存起始地址
    ,uint16_t Des_W      // 目的图像的宽度
    ,uint16_t XDes       // 目的图像的左上方X坐标
    ,uint16_t YDes       // 目的图像的左上方Y坐标
    ,uint32_t ROP_Code     // 光栅操作模式
    /*ROP_Code :
       0000b    0(Blackness)
       ...
       1110b    S0+S1
       1111b    1(whiteness)*/
    ,uint16_t X_W       // 活动窗口的宽度
    ,uint16_t Y_H       // 活动窗口的长度
)
{
    BTE_S0_Color_16bpp();
    BTE_S0_Memory_Start_Address(S0_Addr);
...
void LT_W25N01GV_Read(uint8_t *pBuffer,uint32_t ReadAddr,uint16_t NumByteToRead)//读2k以内数据，从页0位置开始读
{
