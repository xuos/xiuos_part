...
{
    uint32_t spi_baudr = SysctlClockGetFreq(SYSCTL_CLOCK_SPI0 + spi_num) / spi_clk;
    if(spi_baudr < 2 )
    {
        spi_baudr = 2;
    }
    ...
    return SysctlClockGetFreq(SYSCTL_CLOCK_SPI0 + spi_num) / spi_baudr;
}
...
{
    configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
    uint8_t *v_buf = malloc(CmdLen + tx_len);
    size_t i;
    for(i = 0; i < CmdLen; i++)
        v_buf[i] = CmdBuff[i];
    for(i = 0; i < tx_len; i++)
        v_buf[CmdLen + i] = tx_buff[i];

    spi_send_data_normal(spi_num, chip_select, v_buf, CmdLen + tx_len);
    free((void *)v_buf);
}
                                ...
                                const uint8_t *CmdBuff, size_t CmdLen, const uint8_t *tx_buff, size_t tx_len)
{
    ...
    {
        case SPI_TRANS_INT:
            buf = malloc(CmdLen + tx_len);
            for(i = 0; i < CmdLen / 4; i++)
                buf[i] = ((uint32_t *)CmdBuff)[i];
            for(i = 0; i < tx_len / 4; i++)
                buf[CmdLen / 4 + i] = ((uint32_t *)tx_buff)[i];
            v_send_len = (CmdLen + tx_len) / 4;
            break;
        case SPI_TRANS_SHORT:
            buf = malloc((CmdLen + tx_len) / 2 * sizeof(uint32_t));
            for(i = 0; i < CmdLen / 2; i++)
                buf[i] = ((uint16_t *)CmdBuff)[i];
            for(i = 0; i < tx_len / 2; i++)
                buf[CmdLen / 2 + i] = ((uint16_t *)tx_buff)[i];
            v_send_len = (CmdLen + tx_len) / 2;
            break;
        default:
            buf = malloc((CmdLen + tx_len) * sizeof(uint32_t));
            for(i = 0; i < CmdLen; i++)
                buf[i] = CmdBuff[i];
            for(i = 0; i < tx_len; i++)
                buf[CmdLen + i] = tx_buff[i];
            v_send_len = CmdLen + tx_len;
            break;
    }
                               ...
                               size_t CmdLen, uint8_t *rx_buff, size_t rx_len)
{
    ...
    uint32_t i = 0;
    size_t v_cmd_len = CmdLen / frame_width;
    ...
    while (v_cmd_len)
    {
        ...
        switch(frame_width)
        {
            case SPI_TRANS_INT:
                for (index = 0; index < fifo_len; index++)
                    spi_handle->dr[0] = ((uint32_t *)CmdBuff)[i++];
                break;
            case SPI_TRANS_SHORT:
                for (index = 0; index < fifo_len; index++)
                    spi_handle->dr[0] = ((uint16_t *)CmdBuff)[i++];
                break;
            default:
                for (index = 0; index < fifo_len; index++)
                    spi_handle->dr[0] = CmdBuff[i++];
                break;
        }
        spi_handle->ser = 1U << chip_select;
        v_cmd_len -= fifo_len;
...
void spi_receive_data_normal_dma(dmac_channel_number_t dma_send_channel_num,
                                  dmac_channel_number_t dma_receive_channel_num,
                                  spi_device_num_t spi_num, spi_chip_select_t chip_select, const void *CmdBuff,
                                  size_t CmdLen, void *rx_buff, size_t rx_len)
{
    configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
    ...
    if(CmdLen)
        dmac_set_single_mode(dma_send_channel_num, CmdBuff, (void *)(&spi_handle->dr[0]), DMAC_ADDR_INCREMENT, DMAC_ADDR_NOCHANGE,
                           DMAC_MSIZE_4, DMAC_TRANS_WIDTH_32, CmdLen);
    if(CmdLen == 0 && spi_get_frame_format(spi_num) == SPI_FF_STANDARD)
                                   ...
                                   size_t CmdLen, uint8_t *rx_buff, size_t rx_len)
{
    ...
    {
        case SPI_TRANS_INT:
            write_cmd = malloc(CmdLen + rx_len);
            for(i = 0; i < CmdLen / 4; i++)
                write_cmd[i] = ((uint32_t *)CmdBuff)[i];
            ReadBuf = &write_cmd[i];
            v_recv_len = rx_len / 4;
            v_cmd_len = CmdLen / 4;
            break;
        case SPI_TRANS_SHORT:
            write_cmd = malloc((CmdLen + rx_len) /2 * sizeof(uint32_t));
            for(i = 0; i < CmdLen / 2; i++)
                write_cmd[i] = ((uint16_t *)CmdBuff)[i];
            ReadBuf = &write_cmd[i];
            v_recv_len = rx_len / 2;
            v_cmd_len = CmdLen / 2;
            break;
        default:
            write_cmd = malloc((CmdLen + rx_len) * sizeof(uint32_t));
            for(i = 0; i < CmdLen; i++)
                write_cmd[i] = CmdBuff[i];
            ReadBuf = &write_cmd[i];
            v_recv_len = rx_len;
            v_cmd_len = CmdLen;
            break;
    ...
    spi_receive_data_normal_dma(dma_send_channel_num, dma_receive_channel_num, spi_num, chip_select, write_cmd, v_cmd_len, ReadBuf, v_recv_len);

    switch(frame_width)
    {
        case SPI_TRANS_INT:
            for(i = 0; i < v_recv_len; i++)
                ((uint32_t *)rx_buff)[i] = ReadBuf[i];
            break;
        case SPI_TRANS_SHORT:
            for(i = 0; i < v_recv_len; i++)
                ((uint16_t *)rx_buff)[i] = ReadBuf[i];
            break;
        default:
            for(i = 0; i < v_recv_len; i++)
                rx_buff[i] = ReadBuf[i];
                                  ...
                                  size_t CmdLen, uint8_t *rx_buff, size_t rx_len)
{
    ...
    if(frame_width == SPI_TRANS_INT)
        spi_receive_data_normal_dma(dma_send_channel_num, dma_receive_channel_num, spi_num, chip_select, CmdBuff, CmdLen, rx_buff, v_recv_len);
    else
        spi_receive_data_normal_dma(dma_send_channel_num, dma_receive_channel_num, spi_num, chip_select, write_cmd, CmdLen, ReadBuf, v_recv_len);
                                ...
                                const uint32_t *CmdBuff, size_t CmdLen, const uint8_t *tx_buff, size_t tx_len)
{
    ...
    {
        case SPI_TRANS_INT:
            buf = malloc(CmdLen * sizeof(uint32_t) + tx_len);
            for(i = 0; i < CmdLen; i++)
                buf[i] = CmdBuff[i];
            for(i = 0; i < tx_len / 4; i++)
                buf[CmdLen + i] = ((uint32_t *)tx_buff)[i];
            v_send_len = CmdLen + tx_len / 4;
            break;
        case SPI_TRANS_SHORT:
            buf = malloc(CmdLen * sizeof(uint32_t) + tx_len / 2 * sizeof(uint32_t));
            for(i = 0; i < CmdLen; i++)
                buf[i] = CmdBuff[i];
            for(i = 0; i < tx_len / 2; i++)
                buf[CmdLen + i] = ((uint16_t *)tx_buff)[i];
            v_send_len = CmdLen + tx_len / 2;
            break;
        default:
            buf = malloc((CmdLen + tx_len) * sizeof(uint32_t));
...
{
    volatile spi_t *spi_handle = spi[2];
    uint32_t DataWidth = g_instance.data_bit_length / 8;
    ...
    spi_handle->rxftlr = 0x08 / DataWidth - 1;
...
static void spi_slave_command_mode(void)
{
    ...
    case SPI_TRANS_INT:
        for (uint32_t i = 0; i < 8 / 4; i++)
            ((uint32_t *)CmdData)[i] = spi_handle->dr[0];
        break;
    case SPI_TRANS_SHORT:
        for (uint32_t i = 0; i < 8 / 2; i++)
            ((uint16_t *)CmdData)[i] = spi_handle->dr[0];
        break;
    default:
        for (uint32_t i = 0; i < 8; i++)
            CmdData[i] = spi_handle->dr[0];
        break;
    }
    ...
    g_instance.command.cmd = CmdData[0];
    g_instance.command.addr = CmdData[1] | (CmdData[2] << 8) | (CmdData[3] << 16) | (CmdData[4] << 24);
    g_instance.command.len = CmdData[5] | (CmdData[6] << 8);
    ...
    {
        spi_handle->ctrlr0 = (0x0 << g_instance.work_mode) | (0x1 << g_instance.slv_oe) | ((g_instance.data_bit_length - 1) << g_instance.dfs);
        spi[2]->rxftlr = g_instance.command.len / DataWidth - 1;
        spi_handle->imr = 0x00;
        spi_handle->ssienr = 0x01;
    }
...
void spi_slave_config(uint8_t int_pin, uint8_t ready_pin, dmac_channel_number_t dmac_channel, size_t data_bit_length, uint8_t *data, uint32_t len, spi_slave_receive_callback_t callback)
{
    ...
    uint32_t DataWidth = data_bit_length / 8;
    volatile spi_t *spi_handle = spi[2];
    spi_handle->ssienr = 0x00;
