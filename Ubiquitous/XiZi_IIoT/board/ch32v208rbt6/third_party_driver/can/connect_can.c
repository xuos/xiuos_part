...
static struct CanSendConfigure can_send_deconfig = {.stdid = 0x12, .exdid = 0x12, .ide = 0, .rtr = 0, .data_lenth = 8};
...
static void CanGPIOInit(void) {
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO | RCC_APB2Periph_GPIOA,
                           ENABLE);  // 如果CAN引脚映射到PA，需要设置A时钟源，否则设置成B时钟源
...
static uint32 CanModeInit(void *drv, struct BusConfigureInfo *configure_info) {
	...
	/* Under the following settings, the board receive any frames */
    ...
    can1_filter.CAN_FilterFIFOAssignment = CAN_Filter_FIFO0; // must correspond with CAN_MessagePending(CAN1, CAN_FIFO0)
...
static uint32 CanRecvMsg(void *dev, struct BusBlockReadParam *databuf) {
    ...
    for (i = 0; i < msg.DLC; i++) buf[i] = msg.Data[i];
    databuf->size = msg.DLC;

    return msg.DLC;
}

static struct CanDevDone dev_done = {.open = NONE, .close = NONE, .write = CanSendMsg, .read = CanRecvMsg};
...
void CAN1_RX0_IRQHandler(void) {
    ...
    for (i = 0; i < 8; i++) KPrintf("rxbuf [%d] = :%d", i, rxmsg.Data[i]);
}
...
int InitHwCan(void) {
    ...
    ret = HwCanDeviceAttach(CAN_BUS_NAME, CAN_DEVICE_NAME_1);
    if (EOK != ret) {
