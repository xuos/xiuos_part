...
static uint32 SerialInit(struct SerialDriver *serial_drv, struct BusConfigureInfo *configure_info) {
    ...
    struct SerialCfgParam *serial_cfg = (struct SerialCfgParam *)serial_drv->private_data;
    // struct UsartHwCfg *serial_hw_cfg = (struct UsartHwCfg
    // *)serial_cfg->hw_cfg.private_data;
    ...
    // usart_hardware_flow_rts_config(serial_cfg->hw_cfg.serial_register_base,
    // USART_RTS_DISABLE);
    // usart_hardware_flow_cts_config(serial_cfg->hw_cfg.serial_register_base,
    // USART_CTS_DISABLE);
...
#define COMMAND_LINE_MODE 1              // 命令行模式
    ...
    extern int testLTE;                  // 在test_lte命令时才会执行以下语句
    extern int lteMode;
    /* 键盘每按一个键，ch除了会接收该键的ascii值之外，还会额外接收一个-1，需要将其排除 */
    if (testLTE && lteMode == TRANSPARENT_TRANSMISSION_MODE && ch != -1) {  // 透传模式
        USART_SendData(USART2, ch);
        return '\b';
    } else if (testLTE && lteMode == COMMAND_LINE_MODE && ch != -1) {  // 命令行模式
        void lteRedirectUart(char ch);
        lteRedirectUart(ch);
        return '\b';  // 直接return退格键是为了避免shell对lte中的输入数据进行缓存
    }
#endif
    return ch;
}
