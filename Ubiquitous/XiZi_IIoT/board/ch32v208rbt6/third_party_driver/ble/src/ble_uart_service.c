...
// ble_uart GATT Profile Service UUID
const uint8_t ble_uart_ServiceUUID[ATT_UUID_SIZE] = {0x9F, 0xCA, 0xDC, 0x24, 0x0E, 0xE5, 0xA9, 0xE0,
                                                     0x93, 0xF3, 0xA3, 0xB5, 0x01, 0x00, 0x40, 0x6E};

// Characteristic rx uuid
const uint8_t ble_uart_RxCharUUID[ATT_UUID_SIZE] = {0x9F, 0xCA, 0xDC, 0x24, 0x0E, 0xE5, 0xA9, 0xE0,
                                                    0x93, 0xF3, 0xA3, 0xB5, 0x02, 0x00, 0x40, 0x6E};

// Characteristic tx uuid
const uint8_t ble_uart_TxCharUUID[ATT_UUID_SIZE] = {0x9F, 0xCA, 0xDC, 0x24, 0x0E, 0xE5, 0xA9, 0xE0,
...
static gattAttribute_t ble_uart_ProfileAttrTbl[] = {
    ...
    {{ATT_BT_UUID_SIZE, characterUUID}, GATT_PERMIT_READ, 0, &ble_uart_RxCharProps},

    // Characteristic Value 1
    {{ATT_UUID_SIZE, ble_uart_RxCharUUID}, GATT_PERMIT_WRITE, 0, &ble_uart_RxCharValue[0]},

    // Characteristic 2 Declaration
    {{ATT_BT_UUID_SIZE, characterUUID}, GATT_PERMIT_READ, 0, &ble_uart_TxCharProps},

    // Characteristic Value 2
    {{ATT_UUID_SIZE, ble_uart_TxCharUUID}, 0, 0, (uint8_t *)&ble_uart_TxCharValue},

    // Characteristic 2 User Description
    {{ATT_BT_UUID_SIZE, clientCharCfgUUID}, GATT_PERMIT_READ | GATT_PERMIT_WRITE, 0, (uint8_t *)ble_uart_TxCCCD},
 ...
 * LOCAL FUNCTIONS
 */
static bStatus_t ble_uart_ReadAttrCB(uint16_t connHandle, gattAttribute_t *pAttr, uint8_t *pValue, uint16_t *pLen, uint16_t offset,
                                     uint16_t maxLen, uint8_t method);
static bStatus_t ble_uart_WriteAttrCB(uint16_t connHandle, gattAttribute_t *pAttr, uint8_t *pValue, uint16_t len, uint16_t offset,
                                      uint8_t method);
...
bStatus_t ble_uart_add_service(ble_uart_ProfileChangeCB_t cb) {
    ...
    status = GATTServApp_RegisterService(ble_uart_ProfileAttrTbl, GATT_NUM_ATTRS(ble_uart_ProfileAttrTbl), GATT_MAX_ENCRYPT_KEY_SIZE,
                                         &ble_uart_ProfileCBs);
    if (status != SUCCESS) PRINT("Add ble uart service failed!\n");
    ble_uart_AppCBs = cb;
