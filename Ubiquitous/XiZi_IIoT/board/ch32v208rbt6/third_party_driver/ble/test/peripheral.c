...
// Minimum connection interval (units of 1.25ms, 10=12.5ms, default 8)
#define DEFAULT_DESIRED_MIN_CONN_INTERVAL 10

// Maximum connection interval (units of 1.25ms, 100=125ms, default 20)
#define DEFAULT_DESIRED_MAX_CONN_INTERVAL 400

// Slave latency to use parameter update
#define DEFAULT_DESIRED_SLAVE_LATENCY 0

// Supervision timeout value (units of 10ms, 100=1s, default 100)
...
static uint8_t scanRspData[] = {
    ...
    GAP_ADTYPE_LOCAL_NAME_COMPLETE, 'c', 'h', '3', '2', 'v', '2', '0', '8', '_', 'b', 'l', 'e',
...
// GAP Role Callbacks
static gapRolesCBs_t Peripheral_PeripheralCBs = {peripheralStateNotificationCB,  // Profile State Change Callbacks
...
uint16_t Peripheral_ProcessEvent(uint8_t task_id, uint16_t events) {
    ...
    if (events & SBP_PARAM_UPDATE_EVT) {
        ...
        GAPRole_PeripheralConnParamUpdateReq(peripheralConnList.connHandle, DEFAULT_DESIRED_MIN_CONN_INTERVAL,
                                             DEFAULT_DESIRED_MAX_CONN_INTERVAL, DEFAULT_DESIRED_SLAVE_LATENCY, DEFAULT_DESIRED_CONN_TIMEOUT,
                                             Peripheral_TaskID);
 ...
 * @return  none
 */
static void peripheralRssiCB(uint16_t connHandle, int8_t rssi) { KPrintf("RSSI -%d dB Conn  %x \n", -rssi, connHandle); }
...
static void peripheralStateNotificationCB(gapRole_States_t newState, gapRoleEvent_t *pEvent) {
    switch (newState & GAPROLE_STATE_ADV_MASK) {
        ...
        case GAPROLE_ADVERTISING:
            ...
            KPrintf("Ble Advertising..\n");
            break;

        case GAPROLE_CONNECTED:
            if (pEvent->gap.opcode == GAP_LINK_ESTABLISHED_EVENT) {
                Peripheral_LinkEstablished(pEvent);
                KPrintf("Ble Connected..\n");
            }
            break;

        case GAPROLE_CONNECTED_ADV:
            KPrintf("Ble Connected Advertising..\n");
            break;

        case GAPROLE_WAITING:
            if (pEvent->gap.opcode == GAP_END_DISCOVERABLE_DONE_EVENT) {
                KPrintf("Ble Waiting for advertising..\n");
            } else if (pEvent->gap.opcode == GAP_LINK_TERMINATED_EVENT) {
                Peripheral_LinkTerminated(pEvent);
                KPrintf("Ble Disconnected.. Reason:%x\n", pEvent->linkTerminate.reason);
 ...
 * @param  data   数据指针
 * @param  length 数据长度
 * @return bStatus_t 通知状态
 */
bStatus_t peripheralNotify(uint8_t *data, int length) {
    static attHandleValueNoti_t noti;
    uint8_t result = 0xff;
    if (!ble_uart_notify_is_ready(peripheralConnList.connHandle)) {
        return bleNotReady;
    }
    // int len = length < ATT_GetMTU(peripheralConnList.connHandle) - 3 ? length : ATT_GetMTU(peripheralConnList.connHandle) - 3;
    int len = length; // iOS的微信小程序不支持MTU协定
    noti.len = len;
    noti.pValue = GATT_bm_alloc(peripheralConnList.connHandle, ATT_HANDLE_VALUE_NOTI, noti.len, NULL, 0);
    if (noti.pValue != NULL) {
        for (int i = 0; i < len; i++) {
            noti.pValue[i] = data[i];
        }
        result = ble_uart_notify(peripheralConnList.connHandle, &noti, 0); // 通知
        if (result != SUCCESS) {
            KPrintf("R1:%02x\r\n", result);
            GATT_bm_free((gattMsg_t *)&noti, ATT_HANDLE_VALUE_NOTI);
            return result;
            ...
            KPrintf("--------------------------------\n");
            KPrintf("peripheralNotify success, len:%d\n", noti.len);
            KPrintf("noti.pValue: ");
            for (int i = 0; i < noti.len; i++) {
                KPrintf("%02x ", noti.pValue[i]);
                if (i % 10 == 0 && i != 0) {
                    KPrintf("\n");
                }
            }
