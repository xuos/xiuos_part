# 从零开始构建矽璓工业物联操作系统：使用risc-v架构的xidatong-riscv64开发板
...
将上述解压的编译工具链的路径添加到board/xidatong-riscv64/config.mk文件当中，例如：
...
连接开发板串口（即Type-C口）到USB接口，短接ISP模式线，重新上电，然后使用K-Flash工具进行烧写bin文件。
